...
extern uint8_t MFRC522_ReadCardSerial(uint8_t *str);
extern uint8_t MFRC522_isCard(uint8_t *TagType);
...
uint8_t MFRC522_Check(uint8_t *id)
{
	...
	if (status == MI_OK)					   // Card detected. Anti-collision, return card serial number 4 bytes
		return 1;
	else
	{
		return 0;
